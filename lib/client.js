window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function StepRow({ childTask, dependencyLabels, step, workerLabel, workers, t }) {
			const terminalChild = step.status === "completed" || childTask === void 0 || childTask.status === "running" ? void 0 : childTask;
			const visibleStatus = terminalChild?.status ?? step.status;
			const visibleStatusLabel = terminalChild === void 0 ? t(STEP_STATUS_KEYS[step.status]) : t(TASK_STATUS_KEYS[terminalChild.status]);
			const visibleStatusDot = terminalChild === void 0 ? stepDot(step.status) : taskDot(terminalChild.status);
